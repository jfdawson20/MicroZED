...
struct packetDescriptorNode* CreateDescriptorList(uint32_t size,uint32_t buffersize)
{
	...
	{	
		...
		if(i == (size-1))
		{	
			(*temp->packetDescriptor) = ((uint32_t)frameBuffer | 0x00000002);
		}
		else
		{
			(*temp->packetDescriptor) = (uint32_t)frameBuffer;
		}
...
int32_t OcmMallocInit (uint32_t baseaddr,uint32_t size)
{
	ocmBufferPointer = baseaddr;
	ocmBufferLimit   = baseaddr + size;
	return(0);	
}


void* OcmMalloc (uint32_t size)
{
	uint32_t pointer = ocmBufferPointer;
	if((pointer + size) > ocmBufferLimit)
	{
		return(NULL);
	}
	else
	{	
		ocmBufferPointer = (ocmBufferPointer + size);
		return((void*)(ocmBufferPointer));
	}
}
